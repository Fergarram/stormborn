...
create_object({
    ...
    create(self) {
        ...
        self.image_index = Math.floor(Math.random() * 5) + 4;
    }
});
...
function reset_game() {
    ...
    const preview = Object.values(room_current().instances)
        .find(inst => inst.object_id === "obj_preview");
    if(preview) {
        preview.image_alpha = PREVIEW_ALPHA;
    }
...
function reset_game_completely() {
    ...
    ["snd_place", "snd_replace", "snd_tic", "snd_scroll", "snd_win", "snd_scrub"].forEach(sound_id => {
        stop_sound(sound_id);
    });
    ...
    controller.current_turn = 1;
    controller.can_restart = true;
    controller.click_processed = false;
    controller.color_change_processed = false;
    controller.color_change_cooldown = 0;
    controller.opacity_change_processed = false;
    controller.current_opacity = 1;
    controller.player_colors = [0, 1]; 
    ...
    if(back) {
        back.image_speed = 0;
        back.image_index = Math.floor(Math.random() * 5) + 4;
    }
    
    if(ground) {
        ground.image_speed = 0;
        ground.image_index = Math.floor(Math.random() * 5) + 4;
    }

    const sound_controller = instance_get("sound_controller");
    if(sound_controller) {
        sound_controller.click_processed = false;
        sound_controller.has_played_win = false;
        sound_controller.last_back_index = back ? back.image_index : 0;
        sound_controller.last_ground_index = ground ? ground.image_index : 0;
    }


    const timer = instance_get("timer");
    if(timer) {
        timer.inactivity_time = 0;
        timer.last_input_time = 0;
    }


    const old_win_checks = window.requestAnimationFrame(() => {
        window.cancelAnimationFrame(old_win_checks);
    });
}
...
create_object({
    ...
    step(dt, self) {
        if(self.color_change_cooldown > 0) self.color_change_cooldown--;
            ...
            // 
if (!self.prev_r_pressed && (gm.keys_pressed.r || gm.keys_pressed.R)) {
    reset_game_completely();
}
self.prev_r_pressed = (gm.keys_pressed.r || gm.keys_pressed.R);
