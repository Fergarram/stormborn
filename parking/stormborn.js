...
function create_game(config) {
	...
	function draw_sprite(instance) {
		...
		const camera = room.camera;
		const bounds = {
			left: instance.x - sprite.origin_x * instance.image_scale_x,
			right: instance.x + (sprite.frame_width - sprite.origin_x) * instance.image_scale_x,
			top: instance.y - sprite.origin_y * instance.image_scale_y,
			bottom: instance.y + (sprite.frame_height - sprite.origin_y) * instance.image_scale_y,
		};
		const margin = 50;
		if (
			bounds.right < camera.x - margin ||
			bounds.left > camera.x + camera.width + margin ||
			bounds.bottom < camera.y - margin ||
			bounds.top > camera.y + camera.height + margin
		) {
...
export { stormborn_default as default };
