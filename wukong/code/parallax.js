create_object({
    ...
    create(self) {
        ...
        self.layer_instances = self.layers.map((layer) => ({
            ...
            first: {
                x: 0,
                instance: instance_create("obj_background_part", 0, 0, -1000)
            },
            second: {
                x: 280,
                instance: instance_create("obj_background_part", 280, 0, -1000)
            }
        }));
    ...
    step(dt, self) {
        if (!self.controller.game_over) {
            ...
            self.layer_instances.forEach((layer) => {
                ...
                if (layer.first.x <= -280) {
                    layer.first.x = layer.second.x + 280;
                }
                if (layer.second.x <= -280) {
                    layer.second.x = layer.first.x + 280;
                }
            });
        }
    }
...
create_object({
    ...
    create(self) {
        ...
        instance_ref("floor_second", self.second_floor);
    ...
    step(dt, self) {
        if (!self.controller.game_over) {
            ...
            if (self.x <= -280) {
                self.x = self.second_floor.x + 280;
            }
            if (self.second_floor.x <= -280) {
                self.second_floor.x = self.x + 280;
            }
        }
    }
});
