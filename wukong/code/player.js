function jump_key_pressed() {
    return gm.keydown.Space || gm.keydown["8"] || gm.keydown.ArrowUp || gm.keydown["A"];
}

function fall_key_pressed() {
    return gm.keydown.ArrowDown || gm.keydown.ArrowRight || gm.keydown["0"] || gm.keydown["B"];
}
...
create_object({
    ...
    step(dt, self) {
        if (!self.controller.game_over) {
            ...
            if (self.y >= room.height + self.collision_mask.geom[1] - 18) {
                self.y = room.height + self.collision_mask.geom[1] - 18;
                self.on_ground = true;
                self.vertical_speed = 0;
            } else {
                self.on_ground = false;
