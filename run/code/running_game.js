...
create_object({
    ...
    step(dt, self) {
        ...
        let keyPressed = null;
        if (gm.keys_pressed[self.keys[0]] && !self.lastKeyPressed) {
            keyPressed = self.keys[0];
        } else if (gm.keys_pressed[self.keys[1]] && !self.lastKeyPressed) {
            keyPressed = self.keys[1];
        }

        if (keyPressed) {
            self.speed = Math.min(self.speed + self.acceleration, self.maxSpeed);
            self.lastKeyPressed = keyPressed;
            self.animationSpeed = self.speed / 2; // Adjusted to match the new speed range
        } else {
